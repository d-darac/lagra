...
var wipeCmd = &cobra.Command{
	Use:   "wipe [all]",
	Short: "Use to wipe groups created by lagra-mock",
...
func init() {
	GroupsCmd.AddCommand(wipeCmd)
	wipeCmd.Flags().StringSlice("ids", []string{}, "list of group IDs to delete")
}
...
func wipe(ctx context.Context, IDs []id.ID) error {
	...
	for _, groupID := range IDs {
		_, err := s.Get(ctx, groups.GetParams{AccountID: shared.AccountID, GroupID: groupID})
		if err != nil {
			return err
		}
		if err := s.Delete(ctx, tx, groups.DeleteParams{
...
func wipeAll(ctx context.Context) error {
	...
	sql := "delete from groups where account_id = $1 and name ~~* CONCAT('%', $2::text, '%')"
